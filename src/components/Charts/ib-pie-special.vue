<template>
    <div ref="production_pie2" style="width:100%;height:100%">
...
export default ({
    ...
    methods: {
        drawPie() {
            var myChart = echarts.init(this.$refs.production_pie2);
            let that = this
            var option;
            option = {
                tooltip: {
    ...
    computed:{
        ...
        detailArr(){
            let arr = []
            this.bazaarDatatest.forEach(item=>{
                let tou = 1;
                item.itemArr.forEach(item2=>{
                    tou -= 0.1
                    let newColor = JSON.parse(JSON.stringify(item.itemStyle))
                    newColor.opacity = tou
                    let obj = {
                        name:item2.name,
                        value:item2.value,
                        itemStyle:newColor
